...
{
    return[self doCipherkey:key context:kCCEncrypt];
}
- (NSString *) decryptDESWithKey:(NSString *)key
{
    return[self doCipherkey:key context:kCCDecrypt];
}
...
- (NSString *)doCipherkey:(NSString *)key
    ...
    if (cryptStatus == kCCSuccess) {
        return [[NSData dataWithBytesNoCopy:buffer
                                     length:numBytesEncrypted] base64EncodedString];
    }
    free(buffer);
    return nil;
}
...
- (NSString *)encrypt:(NSString *)sText encryptOrDecrypt:(CCOperation)encryptOperation key:(NSString *)key
{
    ...
    {
        //解码 base64
        NSData *decryptData = [sText base64DecodedData];//转成utf-8并decode
